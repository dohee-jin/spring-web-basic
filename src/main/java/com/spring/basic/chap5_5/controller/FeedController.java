...
public class FeedController {
    ...
    // 피드 등록 요청
    @PostMapping
    public ResponseEntity<?> createFeed(@RequestBody @Valid FeedCreateRequest dto) {

        // repository 에게 저장을 위임
        feedService.createProcess(dto);

        return ResponseEntity.ok("피드 저장 성공");
    }

    // 피드 삭제 요청: Delete - api/v5-5/feeds/{id}
    @DeleteMapping("/{id}")
    ...
    // 피드 개별조회 요청: Get - api/v5-5/feeds/{id}
    @GetMapping("/{id}")
    public ResponseEntity<?> feed(@PathVariable Long id) {
